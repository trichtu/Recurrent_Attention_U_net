...
class Recurr_Com_Att_U_Net(nn.Module):
    def __init__(self,img_ch=46,output_ch=2):
        ...
        self.Conv_rain = nn.Conv2d(16,1,kernel_size=1,stride=1,padding=0)

    def forward(self,x,state2_pre=None,state3_pre=None,state4_pre=None,state5_pre=None):
        # encoding path
        x1 = self.Conv1(x)
        ...
        rain = self.Conv_rain(d2)*out[:,0:1,:,:]
        return out, rain, state2, state3, state4, state5


def recurrent_model(model, input):
    '''
    ...
    for i in range(input.size()[1]):
        if i==0:
            out[:,i,:,:,:],rain[:,i,:,:,:],state2,state3,state4,state5 = model(input[:,i,:,:,:])
        else:
            out[:,i,:,:,:],rain[:,i,:,:,:],state2,state3,state4,state5 = model(input[:,i,:,:,:],state2,state3,state4,state5)
